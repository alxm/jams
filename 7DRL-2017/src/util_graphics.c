...
static void loadDir(const char* KeyPrefix, const ASprite* Sheet, int X, int Y, unsigned Speed)
{
    ...
    ASprite* probeSprite = a_sprite_newFromSprite(Sheet, X, Y);
    int height = a_sprite_getHeight(probeSprite) + 1;
...
void z_graphics_load(void)
{
    ...
    ASprite* sh = a_sprite_newFromData(gfx_sprites, "sprites");
    ...
    z_fonts.lightOrange = a_font_dup(A_FONT_FACE_WHITE, a_pixel_hex(0xff9422));
    z_fonts.lightBlue = a_font_dup(A_FONT_FACE_WHITE, a_pixel_hex(0x5ac3f9));
}
...
{
    return a_spriteframes_getNum(Graphic->frames);
}

ASprite* z_graphics_getFrame(const ZGraphic* Graphic, unsigned Frame)
{
    return a_spriteframes_getIndex(Graphic->frames, Frame);
}
