...
size_t z_comp_phone_size(void)
{
    return sizeof(ZCompPhone);
}

void z_comp_phone_init(ZCompPhone* Phone)
{
    Phone->ringing = false;
}

bool z_comp_phone_isRinging(const ZCompPhone* Phone)
{
    return Phone->ringing;
}

void z_comp_phone_setRinging(ZCompPhone* Phone, bool IsRinging)
{
    Phone->ringing = IsRinging;
}
